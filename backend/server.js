...
const symbolsToTrack = {
    // Kim loại quý
    'OANDA:XAU_USD': 'Vàng (XAU/USD)',
    'OANDA:XAG_USD': 'Bạc (XAG/USD)',
    ...
    'OANDA:USD_SGD': 'USD/SGD',
    'OANDA:USD_HKD': 'USD/HKD',
    'OANDA:USD_MXN': 'USD/MXN',
    'OANDA:EUR_PLN': 'EUR/PLN',

    // Tiền điện tử & Chỉ số
    'BINANCE:BTCUSDT': 'Bitcoin (BTC/USDT)',
    'BINANCE:ETHUSDT': 'Ethereum (ETH/USDT)',
    'OANDA:DE30_EUR': 'DAX 30 (Germany)',
    'OANDA:US30_USD': 'Dow Jones 30 (US)',
};
...
app.get('/api/alerts', (req, res) => {
    const alertsWithNames = activeAlerts.map(alert => ({
        ...alert,
        displayName: symbolsToTrack[alert.symbol] || alert.symbol
    }));
...
async function sendTelegramMessage(message) {
    ...
    try {
        ...
        console.log(`[telegram] Đã gửi thông báo: ${message.split('\n')[1]}`);
    } catch (error) {
...
function checkAlerts() {
    activeAlerts.forEach(alert => {
        ...
        if (!prices || prices.price === 0) return;
...
// --- KHỞI CHẠY ---
app.listen(PORT, () => {
    console.log(`🚀 Backend service đang chạy tại http://localhost:${PORT}`);
    connectToFinnhub();
});